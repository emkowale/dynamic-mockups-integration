...
add_filter('woocommerce_email_order_items', 'dmi_add_rendered_image_to_customer_email', 10, 4);

function dmi_add_rendered_image_to_customer_email($html, $order, $is_admin_email, $plain_text) {
    ...
    foreach ($order->get_items() as $item_id => $item) {
        $rendered_url = wc_get_order_item_meta($item_id, '_dmi_rendered_image_url', true);
        if ($rendered_url) {
